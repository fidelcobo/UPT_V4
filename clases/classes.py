...
from clases_pantallas import Ui_not_found
...
class ProcesarCsv(QRunnable):
    ...
    def run(self):
        ...
        generacion_csv(self.fich_a, self.light, self)
...
class Mantenimiento(QtWidgets.QDialog, Ui_Maintenance_Dialog):
    ...
    def procesar_oferta(self):
        ...
        if self.backout_button.isChecked():  # Se buscarán los backouts y sus GPLs
            ...
            if self.fich_oferta.text():
                ...
                if self.plantillaLight.isChecked(): # Se verifica si trabajamos con plantilla Light o larga
                    light = True
                else:
                    light = False

                get_csv = ProcesarCsv(self.fich_oferta.text(), light)
    ...
    def finalizar_csv(self, libro1, libro2, carpeta, ok):
